...
3. 为每个地点查询准确的地址、经度(lng)和纬度(lat)，坐标必须精确到小数点后3位，误差不超过0.01度
...
7. 确保JSON格式完全正确，没有语法错误、多余的逗号或引号
8. 检查每个景点的坐标是否与地址完全匹配，避免出现地点和坐标不符的错误
      ...
      "activities": [
        {
          ...
          "lng": 116.404, // 地点经度，精确到小数点后3位
          "lat": 39.915, // 地点纬度，精确到小数点后3位
          "desc": "活动描述",
...
=== 强制规则：
1. 必须为每个景点/餐厅提供准确的经纬度坐标(lng和lat)，坐标必须与地址完全匹配
2. 所有字段必须填写完整，不能为空
3. 返回纯JSON，不要有任何其他内容
4. 不要在JSON中添加注释
`;
